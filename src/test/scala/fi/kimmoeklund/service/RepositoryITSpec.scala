    ...
    (suite("Repository integration tests")(
      test("it should add permission to the database") {
        check(asciiString, Gen.int) { (name, number) =>
          ...
          for {
            ...
            result <- {
              given QuillCtx = quill.get
              repo.add(permission.id, validForm)
            }
      ...
      test("it should add roles to the database") {
        check(asciiString) { name =>
          for {
            ...
            newRole <- ZIO.succeed(
              Role(RoleId.create, s"role-$name", testData.permissions)
            )
            validForm = ValidRoleForm(s"role-$name", testData.permissions.map(_.id))
            role <- {
              given QuillCtx = quill.get
              repo.add(newRole.id, validForm)
            }
      ...
      test("it should get and delete permission from the database") {
        for {
          ...
          permission <- {
            given QuillCtx = quill.get
            repo.add(PermissionId.create, ValidPermissionForm("test permission", 1))
          }
          allPermissions <- {
            given QuillCtx = quill.get
      ...
      test("it should delete roles from the database") {
        for {
          ...
          role <- {
            given QuillCtx = quill.get
            repo.add(RoleId.create, ValidRoleForm("test role", testData.permissions.map(_.id)))
          }
          allRoles <- {
            given QuillCtx = quill.get
