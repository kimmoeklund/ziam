...
trait Effects[R, T]:
  def getEffect: ZIO[R, ErrorCode, List[T]]
  def postEffect(req: Request): ZIO[R, ErrorCode, T]
  def deleteEffect(id: String): ZIO[R, ErrorCode, Unit]
...
case class SimplePage[R, T](path: Path, menu: Menu, functions: Effects[R, T] & Renderer[T]):
  ...
  def httpValue: HttpApp[R, Nothing] = Http.collectZIO[Request] {
    case Method.GET -> this.path / format =>
      functions.getEffect.foldZIO(
    ...
    case Method.GET -> this.path =>
      functions.getEffect.foldZIO(
        e => {
          for {
            _ <- ZIO.logWarning(e.toString)
    ...
    case req @ Method.POST -> this.path =>
      functions
        .postEffect(req)
        .foldCauseZIO(
          { case c: Cause[_] =>
    ...
    case Method.DELETE -> this.path / id =>
      functions
        .deleteEffect(id)
